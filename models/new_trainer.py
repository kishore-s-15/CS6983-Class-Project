...
def multimodal_dataloader(
    arrival, triage, medrecon, vitals, codes, pyxis, xray, tokenizer, batch=1, num_workers=0
):
...
#     if metric == None:
    ...
    for epoch in range(num_epoch):
        ...
        torch.save(model.state_dict(), f"./weights/{model_name}-{task_name}-{epoch}-final.pth")
        with open(f"./weights/{model_name}-loss.pkl", "wb") as file:
