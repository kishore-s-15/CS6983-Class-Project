...
import numpy as np 
import pandas as pd 
from sklearn.metrics import roc_auc_score, average_precision_score, f1_score, roc_curve, precision_recall_curve, auc
...
#     df = df.iloc[:10000] # test
    ...
    df['pyxis'] = df['pyxis'].fillna("The patient did not receive any medications.")
    df['vitals'] = df['vitals'].fillna("The patient had no vitals recorded")
    df['xray_report'] = df['xray_report'].fillna("The xray was not taken.")
    df['codes'] = df['codes'].fillna("The patient had no diagnostic codes recorded.")
    df = df.drop("admission",axis=1)
    df = df.drop("discharge",axis=1)
    # df = df.drop("eddischarge_category",axis=1)
...
def append_multi_bench_MIMIC(df):
    ...
    # )
    edstays = pd.read_csv("./data/combined_edstays.csv")
    ...
    admissions = pd.read_csv("./data/combined_admissions.csv")
    ...
    # generate dictionaries from the icustays dataset to map to our dataset
    icustays = pd.read_csv("./data/combined_icustays.csv")
...
def cut(df, set_type, label_col="eddischarge"):
    ...
    # logger.info(f"Column Names: {list(col_names)}")
    l = []
    for i in col_names:
...
class Tokenizer():
    ...
    def convert(self, l):
        ...
        xray_hf=Dataset.from_pandas(l[7])
        ...
        xray = xray_hf.map(self.tokenize, batched=True, num_proc=NUM_WORKERS)
        ...
        xray.set_format('torch', columns=["input_ids", "attention_mask", "label"])

        return arrival, triage, medrecon, vitals, codes, pyxis, xray
...
def multimodal_dataloader(arrival,triage,medrecon,vitals,codes,pyxis,xray,tokenizer,batch=1, num_workers=0):
...
def load_torch_model(file, device, mode, task):
    ...
    if mode == "multimodal":
        if task =="multitask":
            model_task_specific = ED_classifier(checkpoint="./MedBERT", num_labels=3, input_dim=768, modalities=7, freeze=True)
        if task == "eddispo": 
            model_task_specific = ED_classifier(checkpoint="./MedBERT", num_labels=2, input_dim=768, modalities=7, freeze=True)
    if mode == "single":
        if task == "multitask":            
    ...
    loaded_state_dict = torch.load(file, map_location=device)
    # print(model_task_specific.state_dict().keys())

    # if model_task_specific.__version__ != loaded_state_dict['model_version']:
    #     print("Model versions don't match. Please check and update.")

    # print(loaded_state_dict.items())

    # expected_keys = model_task_specific.state_dict().keys()
    # filtered_state_dict = {k: v for k, v in loaded_state_dict.items() if k in expected_keys}
    # model_task_specific.load_state_dict(filtered_state_dict, strict=False)

    # model_task_specific.to(device)
    model_task_specific.load_state_dict(torch.load(file, map_location=device),strict=False)
...
def inference_eddispo(model_name, model, test_dataloader, device, metric=None, mode='multimodal', plot=False):
    ...
    if metric is None: 
        ...
        metric2 = precision_recall_curve
        #metric3 = load_metric("arthurvqin/pr_auc")
...
if __name__ == '__main__':    
    ...
    DATA = config.get('data', 'C:/Users/Dell/Desktop/ReportGen Project/MEME/text_repr_with_xray.json')
    ...
    DATASET = config.get("dataset", "MIMIC")
    ...
    try:
        ...
        arrival_test_tokens, triage_test_tokens, medrecon_test_tokens, vitals_test_tokens, codes_test_tokens, pyxis_test_tokens,xray_test_tokens= processor.convert(l_all)
